...
export class AdminReportDetailComponent implements OnInit, AfterViewInit {
  ...
  assignToRescuer() {
    ...
    if (!this.assignedRescuer()) {
      this.errorMessage.set('لطفاً یک امدادگر انتخاب کنید.');
      return;
    }
      ...
      this.reportService.updateReport(this.reportId, { assignedTo: this.assignedRescuer(), isSentToRescuer: true }).subscribe({
        next: () => {
          ...
          this.notificationService.addNotification({
            userId: this.assignedRescuer(),
            message: `گزارش جدید "${report.title}" به شما تخصیص داده شد.`,
            isRead: false
          }).subscribe();
  ...
  private loadRescuers() {
    this.userService.getUsers().subscribe(users => {
      this.rescuers = users.filter(user => user.role === 'rescuer');
    });
  }
